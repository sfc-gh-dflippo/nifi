...
/**
 * Imports Parquet into PostgreSQL via COPY FROM STDIN using Apache Parquet directly for PostgreSQL communication. Still uses NiFi RecordReader for
...
public final class ParquetImportStreamer {
    ...
    public static void streamParquetImport(final ProcessSession session, final FlowFile flowFile, final OutputStream out) {
    ...
    public static void transcodeToParquetImport(final ProcessSession session, final FlowFile flowFile, final RecordReaderFactory readerFactory,
            final OutputStream out, final ComponentLog logger) {
        // Use a temporary file to write Parquet data first, then stream to output
        // This avoids loading everything into memory while still providing proper
        // Parquet format
        try {
            ...
            try {
                session.read(flowFile, in -> {
                    try (RecordReader reader = readerFactory.createRecordReader(flowFile, in, logger)) {
                        ...
                        try (ParquetWriter<GenericRecord> writer = AvroParquetWriter.<GenericRecord>builder(outputFile).withSchema(avroSchema)
