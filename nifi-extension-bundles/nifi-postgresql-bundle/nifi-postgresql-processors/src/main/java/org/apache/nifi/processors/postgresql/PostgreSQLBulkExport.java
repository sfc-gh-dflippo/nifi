...
@CapabilityDescription("Exports rows from PostgreSQL using COPY (query) TO STDOUT with CSV HEADER. Supports initial-load and incremental fetching using maximum-value columns with persisted state and optional chunking into multiple FlowFiles. Parses the COPY CSV stream using Apache Commons CSV and writes records using a configured RecordSetWriter. CSV options (delimiter, quote, escape, NULL token) are configurable. Creates/drops a quoted temporary view to wrap custom queries. Sets 'record.count' and 'mime.type' attributes on results.")
@InputRequirement(Requirement.INPUT_FORBIDDEN)
@SupportsBatching
@PrimaryNodeOnly
@Stateful(scopes = Scope.CLUSTER, description = "Stores maximum observed values for configured columns to support incremental fetching across cluster nodes.")
@DefaultSchedule(strategy = SchedulingStrategy.TIMER_DRIVEN, period = "1 min")
@WritesAttributes({
        @WritesAttribute(attribute = "mime.type", description = "MIME type of the output content"),
        @WritesAttribute(attribute = "record.count", description = "Number of records written when conversion occurs")
})
/**
 * Exports rows from PostgreSQL using COPY (query) TO STDOUT. Supports incremental fetching
 * via maximum-value columns persisted in state and optional chunking of results.
 */
public class PostgreSQLBulkExport extends AbstractProcessor {
    ...
    static final Relationship REL_SUCCESS = new Relationship.Builder()
            .name("success")
            .description("FlowFiles containing exported data")
            .build();

    static final Relationship REL_FAILURE = new Relationship.Builder()
            .name("failure")
            .description("FlowFiles that failed during export")
            .build();
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try (PostgreSQLConnectionWrapper wrapper = connectionProvider.getPostgreSQLConnection()) {
            ...
            createTempView(wrapper, emptyViewSql);
            try {
                final String format = properties.getBulkTransferDataFormat();
                final RecordSchema writeSchema = "CSV".equalsIgnoreCase(format) ? fetchViewSchema(wrapper, viewName) : null;
                final String copySql = sqlBuilder.buildCopyFromStatementSql(baseQuery);
                ...
                if (!chunking || (format != null && format.equalsIgnoreCase("Parquet"))) {
                    final CopyStreamUtil.CopyOutContext copyOutContext = CopyStreamUtil.startCopyOut(copyManager, copySql, "nifi-pg-bulkexport-export");

                    final Map<String, String> attributes = new HashMap<>();
                    if ("CSV".equalsIgnoreCase(format)) {
                        FlowFile output = session.create();
                        output = CsvExportStreamer.writeCsvExportToFlowFileWithSchema(session, output, properties, writerFactory, writeSchema, copyOutContext.getInputStream(), getLogger(), attributes, maxTracker);
                        try { copyOutContext.joinAndRethrowIfError(); } catch (RuntimeException e) { throw new ProcessException("COPY TO STDOUT failed", e.getCause() == null ? e : e.getCause()); }
                        updateStateFromTracker(context, maxTracker);
                        ...
                        if (maxRows <= 0) {
                            FlowFile output = session.create();
                            output = ParquetExportStreamer.transcodeParquetExportToWriter(session, output, copyOutContext.getInputStream(), writerFactory, getLogger(), attributes, maxTracker);
                            try { copyOutContext.joinAndRethrowIfError(); } catch (RuntimeException e) { throw new ProcessException("COPY TO STDOUT failed", e.getCause() == null ? e : e.getCause()); }
                            updateStateFromTracker(context, maxTracker);
                            if (!attributes.isEmpty()) {
                                output = session.putAllAttributes(output, attributes);
                            ...
                            final List<FlowFile> parts = ParquetExportStreamer.transcodeParquetExportToWriterChunked(session, working, copyOutContext.getInputStream(), writerFactory, getLogger(), maxRows, maxTracker);
                            if (parts.isEmpty()) {
                                FlowFile output = session.create();
                                output = ParquetExportStreamer.transcodeParquetExportToWriter(session, output, copyOutContext.getInputStream(), writerFactory, getLogger(), attributes, maxTracker);
                                updateStateFromTracker(context, maxTracker);
                    ...
                    final CopyStreamUtil.CopyOutContext copyOutContext2 = CopyStreamUtil.startCopyOut(copyManager, copySql, "nifi-pg-bulkexport-export");
                    try {
                        final CSVFormat csvFormat = CsvFormats.buildCsvParseFormat(properties);
                        try (org.apache.nifi.csv.CSVRecordReader rr = new org.apache.nifi.csv.CSVRecordReader(copyOutContext2.getInputStream(), getLogger(), writeSchema, csvFormat, true, false, null, null, null, StandardCharsets.UTF_8.name())) {
                            List<Record> batch = new ArrayList<>(maxRows);
                            Record rec;
                            while ((rec = rr.nextRecord()) != null) {
                                if (maxTracker != null) maxTracker.observe(rec);
                                batch.add(rec);
                ...
                dropTempViewQuietly(wrapper, viewName);
            }
        } catch (Exception e) {
            getLogger().error("Failed to bulk export from PostgreSQL", e);
    ...
    private void createTempView(final PostgreSQLConnectionWrapper wrapper, final String sql) {
        try (Statement st = wrapper.connection.createStatement()) {
            st.execute(sql);
        } catch (Exception e) {
    ...
    private void dropTempViewQuietly(final PostgreSQLConnectionWrapper wrapper, final String viewName) {
        try (Statement st = wrapper.connection.createStatement()) {
            st.execute("DROP VIEW IF EXISTS " + SqlBuilder.quoteIdentifier(viewName));
        } catch (Exception ignore) { }
    }

    private RecordSchema fetchViewSchema(final PostgreSQLConnectionWrapper wrapper, final String viewName) {
        final List<RecordField> fields = new ArrayList<>();
        final String sql = "select a.attname, format_type(a.atttypid, a.atttypmod) typ " +
                "from pg_attribute a join pg_class c on a.attrelid=c.oid " +
                "where c.relname='" + viewName + "' and a.attnum>0 and not a.attisdropped order by a.attnum";
        try (Statement st = wrapper.connection.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
    ...
    private org.apache.nifi.serialization.record.DataType mapPgTypeToRecordType(final String pgType) {
        if (pgType.startsWith("int2") || pgType.equals("smallint")) return RecordFieldType.SHORT.getDataType();
        if (pgType.startsWith("int4") || pgType.equals("integer")) return RecordFieldType.INT.getDataType();
        if (pgType.startsWith("int8") || pgType.equals("bigint")) return RecordFieldType.LONG.getDataType();
        if (pgType.startsWith("float4") || pgType.equals("real")) return RecordFieldType.FLOAT.getDataType();
        if (pgType.startsWith("float8") || pgType.equals("double precision")) return RecordFieldType.DOUBLE.getDataType();
        if (pgType.startsWith("numeric") || pgType.startsWith("decimal")) return RecordFieldType.DECIMAL.getDataType();
        if (pgType.equals("boolean")) return RecordFieldType.BOOLEAN.getDataType();
        if (pgType.equals("date")) return RecordFieldType.DATE.getDataType();
        if (pgType.startsWith("timestamp")) return RecordFieldType.TIMESTAMP.getDataType();
        if (pgType.equals("time")) return RecordFieldType.TIME.getDataType();
        if (pgType.equals("uuid")) return RecordFieldType.UUID.getDataType();
    ...
    private Map<String, Object> parseCsvRecord(final CSVRecord csvRecord, final RecordSchema writeSchema, final String nullToken) {
        ...
        for (RecordField f : writeSchema.getFields()) {
            ...
            switch (f.getDataType().getFieldType()) {
                ...
                case DECIMAL -> out.put(name, new java.math.BigDecimal(raw));
                case BOOLEAN -> out.put(name, Boolean.valueOf(raw));
                case DATE -> out.put(name, java.sql.Date.valueOf(raw));
                case TIME -> out.put(name, java.sql.Time.valueOf(raw));
                case TIMESTAMP -> out.put(name, java.sql.Timestamp.valueOf(raw.replace('T',' ').replace('Z',' ')));
                case UUID -> out.put(name, java.util.UUID.fromString(raw));
    ...
    protected Collection<ValidationResult> customValidate(final ValidationContext validationContext) {
        ...
        if (!validationContext.getProperty(RECORD_WRITER).isSet()) {
            results.add(new ValidationResult.Builder()
                    .subject("Record Writer")
                    .valid(false)
                    .explanation("Record Writer is required for exporting results")
                    .build());
        }
        return results;
    }
