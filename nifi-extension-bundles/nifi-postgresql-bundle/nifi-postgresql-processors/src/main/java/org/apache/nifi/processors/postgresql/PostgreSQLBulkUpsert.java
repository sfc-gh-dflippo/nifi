...
/**
 * Performs a bulk UPSERT into a target PostgreSQL table by loading a temporary table
 * using COPY FROM STDIN (CSV or Parquet) and executing a caller-provided UPSERT SQL template.
 * Optionally streams results back using COPY (query) TO STDOUT.
 */
@Tags({"postgresql", "copy from", "bulk", "upsert", "merge"})
@CapabilityDescription("Performs bulk upsert to a target table using a temporary table loaded with COPY FROM STDIN in CSV or Parquet format. In CSV mode, a RecordReader parses incoming FlowFiles and Apache Commons CSV generates the COPY stream with configurable options. In Parquet mode, the FlowFile content must be Parquet bytes and is streamed to COPY FROM using pg_parquet with MATCH_BY control. Optionally exports results using COPY (query) TO STDOUT in CSV or Parquet according to Data Format. In CSV result mode, a RecordSetWriter is required; in Parquet result mode, raw Parquet bytes are written. Sets 'record.count' for processed input records in CSV mode and safely quotes identifiers.")
@InputRequirement(Requirement.INPUT_REQUIRED)
@SupportsBatching
@WritesAttributes({
        @WritesAttribute(attribute = "record.count", description = "Number of input records processed for upsert"),
...
public class PostgreSQLBulkUpsert extends AbstractProcessor {
    ...
    // Upsert
    static final PropertyDescriptor UPSERT_SQL_TEMPLATE = ProcessorProperties.UPSERT_SQL_TEMPLATE;
    ...
    static final Relationship REL_SUCCESS = new Relationship.Builder()
            .name("success")
            .description("FlowFiles successfully processed and, if configured, containing exported results")
            .build();

    static final Relationship REL_FAILURE = new Relationship.Builder()
            .name("failure")
            .description("FlowFiles that failed during upsert")
            .build();
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        // Get the raw template without expression language evaluation
        final String upsertTemplate = context.getProperty(UPSERT_SQL_TEMPLATE).getValue();
        ...
        try (PostgreSQLConnectionWrapper wrapper = connectionProvider.getPostgreSQLConnection()) {
            ...
            try (Statement stmt = wrapper.connection.createStatement()) {
            ...
            final RecordSetWriterFactory writerFactory = context.getProperty(RECORD_WRITER).isSet()
            ...
            final String renderedUpsert = upsertTemplate
                    .replace("${temp_table}", quoteIdentifier(tempTable))
                    .replace("${target_table}", quoteIdentifierPath(targetTable));
            
            getLogger().debug("Rendered upsert SQL: {}", renderedUpsert);

            if (upsertReturnsRecords) {
                final String copyOutSql = sqlBuilder.buildCopyFromStatementSql(renderedUpsert);

                final CopyStreamUtil.CopyOutContext copyOutContext = CopyStreamUtil.startCopyOut(copyManager, copyOutSql, "nifi-pg-bulkupsert-result-export");
                ...
                if (maxRowsPerFlowFile <= 0) {
                    ...
                    if ("Parquet".equalsIgnoreCase(format)) {
                        flowFile = ParquetExportStreamer.transcodeParquetExportToWriter(session, flowFile, copyOutContext.getInputStream(), outWriterFactory, getLogger(), attributes, null);
                    } else {
                        flowFile = CsvExportStreamer.writeCsvExportToFlowFile(session, flowFile, properties, outWriterFactory, copyOutContext.getInputStream(), getLogger(), attributes, null);
                    }
                    flowFile = session.putAllAttributes(flowFile, attributes);
                } else {
                    // Chunk results into multiple FlowFiles
                    if ("Parquet".equalsIgnoreCase(format)) {
                        final List<FlowFile> parts = ParquetExportStreamer.transcodeParquetExportToWriterChunked(session, flowFile, copyOutContext.getInputStream(), outWriterFactory, getLogger(), maxRowsPerFlowFile, null);
                        ...
                        try {
                            final List<FlowFile> parts = CsvExportStreamer.writeCsvExportChunked(session, flowFile, properties, outWriterFactory, copyOutContext.getInputStream(), getLogger(), maxRowsPerFlowFile, null);
                            for (FlowFile p : parts) {
                                session.transfer(p, REL_SUCCESS);
                            }
                        } catch (Exception e) {
                ...
                try (Statement stmt = wrapper.connection.createStatement()) {
                    stmt.execute(renderedUpsert);
                }
            }

            try { wrapper.connection.commit(); } catch (Exception e) { throw new ProcessException("Commit failed after UPSERT", e); }
    ...
    protected Collection<ValidationResult> customValidate(final ValidationContext validationContext) {
        ...
        if (returns && !validationContext.getProperty(RECORD_WRITER).isSet()) {
            results.add(new ValidationResult.Builder()
                    ...
                    .valid(false)
                    .explanation("Record Writer is required when exporting results")
                    .build());
        }
        return results;
    }
    ...
    private void transcodeToParquetCopyIn(final ProcessSession session,
        ...
        session.read(flowFile, in -> {
            try (RecordReader reader = readerFactory.createRecordReader(flowFile, in, getLogger());
                 RecordSetWriter writer = writerFactory.createWriter(getLogger(), reader.getSchema(), out, new java.util.HashMap<>())) {
